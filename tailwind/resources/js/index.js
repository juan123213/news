            ...
            <article class="rounded overflow-hidden shadow-lg">
                ...
                <div class="px-6 py-4 flex flex-row items-center">
                    <span href="#" class="py-1 text-sm font-regular text-gray-900 mr-1 flex flex-row items-center">
                        <button data-title="${article.title}" data-content="${article.description}" data-link="${article.url}" class="open-treact-popup close-treact-popup mr-3 px-2 py-2 rounded border border-gray-400 hover:bg-gray-200 transition duration-300"> 
                            <svg height="13px" width="13px" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
    ...
    function openTreactPopup(objPopup){ 
    ...
    window.onclick = function(e){
        ...
        if( e.target.classList.contains('open-treact-popup') ) {
            e.preventDefault();

            var elPopup = document.querySelector(".treact-popup");
            var titlePop = e.target.dataset.title;
            var contentPop = e.target.dataset.content;
            var linkPop = e.target.dataset.link;

            document.querySelector('#popup-title').innerHTML = titlePop;
            document.querySelector('#popup-content').innerHTML = contentPop;
            document.querySelector('#popup-link').setAttribute('href', linkPop);

            openTreactPopup(elPopup);
        }
    }    